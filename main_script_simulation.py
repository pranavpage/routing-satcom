...
threshold_fraction = float(args[9])
...
max_buff_length = int(threshold_fraction*drop_buff_length)
...
def congestion_control(node, path_enhanced, type=cc_type):
    ...
    if(type == 'ekici'):
        ...
        # print(f"Node idx : {node_idx}, path : {path_enhanced.primary}")
    ...
    if(type == 'prob-routing'):
        ...
        if(ngbr_buff_length>0):
            ...
            a = buff_length
        # print(f"{buff_length}, {ngbr_buff_length} -> a : {a:.2f}")
...
class node:
    ...
    def route(self, packet, algo_type=algo_type):
        ...
        if(algo_type=='test'):
            ...
            # print(f"{self.p, self.s}->{packet.p2, packet.s2} : {path_routed}")
    ...
    def execute(self):
        ...
        if(self.event_type=='arrival'):
            ...
            if(not (self.packet.s1 == self.packet.s2 and self.packet.p1 == self.packet.p2)):
                ...
                if(len(source_node.buffers[i][j])<=drop_buff_length):
